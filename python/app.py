...
#from stocks import stockpredict,search
from newprediction import search,stockpredict
...
def displayStock(stockName):
    stockData = []
    with open('static/stocks/'+stockName+'/'+stockName+'.txt', 'w') as filehandle:
        for line in filehandle:
            currentPlace = line[:-1]
            stockData.append(currentPlace)
    return render_template("stockdetails.html", stockName = stockName, stockData=stockData)
